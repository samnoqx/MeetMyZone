...
const POPULAR_SLUGS = ['gmt-to-est', 'utc-to-ist', 'est-to-pst', 'cet-to-est', 'jst-to-aest', 'ist-to-est', 'pst-to-gmt'];

export default function TimezoneConverterHubClient({ pairings }: Props) {
  ...
  // 3. Group all filtered pairings alphabetically by their source abbreviation
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      <div className="relative w-full max-w-xl mx-auto">
        <input
          type="text"
          placeholder="Search timezone (e.g. EST, GMT, UTC)..."
          aria-label="Search timezone converter list"
          value={searchQuery}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {popularPairings.map((p) => (
                <PremiumCard
                  ...
                  description={`Convert ${p.sourceDisplayName} (${p.sourceCityName}) to ${p.targetDisplayName} (${p.targetCityName}) with live clocks, DST shifts, and meeting planner.`}
                  cta={
                    <Link
                      href={`/convert/${p.slug}`}
      ...
      <section className="flex flex-col gap-8 border-t border-border-custom pt-8">
        ...
        {groupedPairings.length === 0 ? (
          <div className="text-center py-10 flex flex-col gap-2">
            <p className="text-sm font-semibold text-txt-primary">No matching timezone converters found</p>
            <p className="text-xs text-txt-muted">Try searching for other abbreviations like UTC, EST, or GMT</p>
          </div>
        ) : (
          <div className="flex flex-col gap-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                   {group.items.map((p) => (
                    <PremiumCard
                      ...
                      className="premium-card p-5 rounded-2xl flex flex-col justify-between transition-all duration-200 hover:border-brand-accent hover:-translate-y-1 hover:shadow-card-hover"
                      title={`${p.sourceDisplayName} to ${p.targetDisplayName}`}
                      description={`Convert ${p.sourceDisplayName} (${p.sourceCityName}) to ${p.targetDisplayName} (${p.targetCityName}) with live clocks and meeting planner.`}
                      cta={
                        <Link
                          href={`/convert/${p.slug}`}
                          className="w-full py-2 mt-4 bg-brand-accent hover:bg-brand-accent-hover text-slate-950 font-extrabold text-xs rounded-lg transition-colors shadow-sm cursor-pointer text-center active:scale-[0.98] border border-slate-950/5 block"
