...
export default function ShareMenu({ cities, matrix, selectedDate, isCompact = false, isMobileGrid = false }: ShareMenuProps) {
  ...
  const handleCopySlackSummary = async () => {
    ...
    return (
      <div className="grid grid-cols-2 gap-3 w-full md:hidden mt-5 mb-4">
        ...
        <a
          href="#world-clock"
          className="h-12 flex items-center justify-center gap-1.5 bg-brand-accent-surface border border-brand-accent-surface-strong shadow-md dark:shadow-lg dark:shadow-black/20 rounded-xl text-center text-sm active:scale-[0.98] active:bg-brand-accent-surface-strong transition-all duration-200 cursor-pointer w-full px-2 text-brand-accent-deep dark:text-brand-accent font-bold"
        >
          <svg className="w-4 h-4 text-brand-accent-deep dark:text-brand-accent shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {toast.show && (
          <div className="fixed bottom-6 right-6 bg-brand-accent text-slate-950 px-4 py-3 rounded-xl shadow-2xl border border-brand-accent-hover z-50 flex items-center gap-2.5 animate-bounce font-extrabold text-xs">
  ...
  return (
    <div className={`flex ${isCompact ? 'flex-row gap-2' : 'flex-row md:flex-row gap-1.5 md:gap-3'} items-center md:items-center w-full md:w-auto`}>
      ...
      <button
        onClick={handleCopyLink}
        className={`flex items-center justify-center md:justify-start gap-2 bg-surf-2 border border-border-custom hover:bg-surf-3 hover:border-brand-accent/30 text-txt-primary hover:text-brand-accent-deep dark:hover:text-brand-accent rounded-lg text-xs md:text-sm transition-all shadow-xs active:scale-95 cursor-pointer font-bold ${
      ...
      {/* Floating Toast Notification */}
      {toast.show && (
        <div className="fixed bottom-6 right-6 bg-brand-accent text-slate-955 px-4 py-3 rounded-xl shadow-2xl border border-brand-accent-hover z-50 flex items-center gap-2.5 animate-bounce font-extrabold text-xs">
          <svg className="w-4 h-4 text-slate-950" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
          </svg>
          {toast.message}
