...
export interface HomeTimePreviewProps {
  initialClock1?: ClockState;
  initialClock2?: ClockState;
  storageKeyPrefix?: string;
}

export default function HomeTimePreview({
  initialClock1,
  initialClock2,
  storageKeyPrefix = 'wc_home'
}: HomeTimePreviewProps) {
  const {
    mounted,
    currentTime,
    clock1,
  ...
  const getDifferenceString = () => {
    ...
    if (diffMinutes === 0) {
      return `${clock1.city} is at the same time as ${clock2.city}`;
    }
    const relation = diffMinutes > 0 ? 'ahead of' : 'behind';
    return `${clock1.city} is ${diffHours} hour${diffHours === 1 ? '' : 's'} ${relation} ${clock2.city}`;
  };
